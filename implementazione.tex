...
\begin{enumerate}
    ...
    \item Effettua il wiring, ovvero collega manualmente gli oggetti in memoria chiamando il metodo \texttt{attach()} sul Subject per ogni Observer trovato (es.\texttt{expense.attach(membership)}).
    \item Esegue la modifica richiesta. A questo punto, il Subject invoca internamente \texttt{notifyObservers()}; le Membership collegate ricevono la notifica in tempo reale.
    \item Parallelamente al sistema di notifiche, il Service si fa carico della logica di business contabile: calcola le quote, aggiorna transazionalmente i Balance in memoria e utilizza i DAO appropriati (es.\ BalanceDAO) per salvare le modifiche definitive sul database, garantendo l'integrità dei dati.
\end{enumerate}
...
\textbf{Debtors} (saldo negativo) e \textbf{Creditors} (saldo positivo),
