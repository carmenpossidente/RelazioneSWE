...
\begin{enumerate}
    ...
    Questo package implementa il pattern Observer per gestire la propagazione dei cambiamenti di stato all'interno del Domain Model, assicurando che quando avviene un’azione (es. una spesa viene creata), le entità interessate ricevano una notifica per reagire di conseguenza (es. simulando l'invio di un avviso o una email all'utente interessato).
    \begin{itemize}
        \item \textbf{Subject (Classe Astratta)}: Definisce il contratto per le entità che possono generare eventi (come Group, Expense, Settlement). Gestisce una lista di observer mantenuta solo in memoria (non persistita a database) e fornisce metodi per l’aggancio (attach) e la notifica (notifyObservers).
        
        \item \textbf{Observer (Interfaccia)}: Definita per le entità che devono reagire ai cambiamenti. Nel sistema, la classe Membership implementa questa interfaccia per intercettare gli eventi di dominio e gestire le notifiche (es. loggando l'invio simulato di una comunicazione all'utente), delegando invece gli aggiornamenti contabili critici (come i Balance) al livello Service per garantirne la transazionalità.
    ...
    \begin{itemize}
        ...
        \item \textbf{Membership (Entity, Observer)}: Classe associativa che lega un User e un Group. Definisce il ruolo (ADMIN, MEMBER) e lo stato (ACTIVE, WAITING\_ACCEPTANCE, REMOVED). Implementa Observer per monitorare i cambiamenti nel gruppo di appartenenza, limitandosi a innescare le opportune notifiche per l'utente associato.
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Composizione}: Group esercita una composizione (1:N) su Membership, Expense e Settlement, indicando che queste entità non hanno ragione di esistere al di fuori del contesto del gruppo. Analogamente, Expense esercita una composizione (1:N) su ExpenseParticipant: questo vincolo indica che i dettagli della partecipazione (come la quota individuale shareAmount) sono parti integranti della spesa stessa e non possono esistere indipendentemente da essa.
    
    \item \textbf{Associazioni}: Un User può possedere più Membership, permettendogli di partecipare a diversi gruppi mantenendo profili e ruoli distinti. Ogni Membership ha esattamente un Balance associato (1:1). Le spese e i pareggi puntano a specifiche istanze di Membership (per i ruoli di payer, creator o beneficiary), questo garantisce la tracciabilità e la coerenza del modello di business, identificando univocamente i soggetti coinvolti nelle transazioni all'interno del gruppo.
\end{itemize}
